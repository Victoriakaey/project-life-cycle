---
description: Cut a new release end-to-end. Computes SemVer bump from CHANGELOG [Unreleased], renames the section, bumps both Claude-plugin manifests, validates, commits, tags, pushes, and verifies the GitHub Release landed. One human checkpoint (confirm bump + version).
---
...
- `[Unreleased]` is empty or contains only `sync:` / chore-quiet items → nothing user-visible to ship; skip.
...
3. Confirm `CHANGELOG.md` + `.claude-plugin/marketplace.json` + `.claude-plugin/plugin.json` + `.github/workflows/release.yml` all exist. Abort if any missing — fall back to plain `git tag` workflow.
...
5. Verify `marketplace.json` plugins[0].version matches. Abort if they disagree — that's the bug the validator catches; fix first.
...
15. Run `python3 scripts/validate.py`. Abort + revert (`git checkout -- .claude-plugin/ CHANGELOG.md`) on any error — surface the validator output.
16. If `./scripts/sync.sh check` exists, run it. Abort + revert on drift.

### Phase 6 — Commit + tag + push

17. `git add CHANGELOG.md .claude-plugin/marketplace.json .claude-plugin/plugin.json`.
18. Commit:
    ```bash
...
26. Print to the user:
    ...
    - Suggested upgrade command for users: `claude plugin marketplace update <marketplace-name> && claude plugin update <plugin-name>`

## Error recovery

- **Validate fails** → `git checkout -- .claude-plugin/ CHANGELOG.md`, surface error, stop.
...
- **Mid-flight cancel** (user hits Ctrl-C between Phase 4 and Phase 6) → run `git checkout -- .claude-plugin/ CHANGELOG.md` to undo the file edits before retrying.
...
- **Running `/release` when `[Unreleased]` is empty or only `sync:` entries** → no user-visible delta; release notes will be empty; users see a "version bump" with no value. Skip the release.
