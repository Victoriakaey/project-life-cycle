// Pure, side-effect-free. Canonical spec of the verify-tail logic. Node-tested.
// The Workflow entry (verify-tail.mjs) consumes these; keep both in sync in ONE commit.
...
export function validateAgainstSchema (obj, schema, path = '$') {
  const errors = []
  const walk = (o, s, p) => {
    if (s.enum && !s.enum.includes(o)) { errors.push(`${p}: ${JSON.stringify(o)} not in ${s.enum.join('|')}`); return }
    if (s.type === 'array') {
      if (!Array.isArray(o)) { errors.push(`${p}: expected array`); return }
      o.forEach((it, i) => walk(it, s.items, `${p}[${i}]`))
      return
    }
    if (s.type === 'object' || s.properties) {
      if (o === null || typeof o !== 'object') { errors.push(`${p}: expected object`); return }
      for (const k of s.required || []) if (!(k in o)) errors.push(`${p}.${k}: required`)
      for (const [k, sub] of Object.entries(s.properties || {})) if (k in o) walk(o[k], sub, `${p}.${k}`)
    }
  }
...
export function buildVerifierPrompt ({ storyPath, builderSummary }) {
  return [
...
export function buildCqPrompt ({ builderSummary, scopePin }) {
  return [
...
export function buildValidatorPrompt ({ storyPath, specPath, planPath, builderSummary, verifierReport, diffRange }) {
  return [
    'ROLE: VALIDATOR.',
...
export function decideLanes (storyPath) {
  const runVerifier = storyPath != null
  return { runVerifier, validatorJoinsOnVerifier: runVerifier }
}

export function cqScopePin (prevTaskTip, builderCommitSHA) {
  return `${prevTaskTip}..${builderCommitSHA}`
}

export function mergeFindings ({ verifier, cq, validator }) {
  const holes = []
...
export function normalizeVerifier (verifierRaw, runVerifier) {
